...
# ১. হোম স্ক্রিন মেনু (যেখানে ইনবক্স বা গডের ফাইলের বাটন সহ সব মেনু থাকবে)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_tasks":
        ...
        if not is_task_active:
            ...
            # একা একা অটো নতুন নাম, জিমেইল ও পাসওয়ার্ড জেনারেট হবে
        ...
        if user_id in user_current_gmail:
            ...
            keyboard = [[InlineKeyboardButton("🔄 জিমেইল নিন", callback_data="task_gmail"), InlineKeyboardButton("⬅️ ব্যাক", callback_data="home")]]
    ...
    # ইনবক্স / গডের ফাইল (God File / Admin Panel) লগইন ও ড্যাশবোর্ড
        ...
        if admin_logged_in.get(user_id, False):
            if not pending_task_submissions:
                keyboard = [[InlineKeyboardButton("⬅️ ব্যাক (ইনবক্স মেনু)", callback_data="admin_dashboard")]]
                ...
                keyboard = [
                    ...
                    [InlineKeyboardButton("⬅️ ব্যাক (ইনবক্স মেনু)", callback_data="admin_dashboard")]
                ]
                text = (
        ...
        await query.edit_message_text(text="✅ জিমেইল কাজটি সফলভাবে **Approve** করা হয়েছে!", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ ব্যাক (ইনবক্স মেনু)", callback_data="admin_dashboard")]]))

    elif data == "task_reject":
        if pending_task_submissions:
            pending_task_submissions.pop(0)
            user_data_db['tasks_rejected'] += 1
        await query.edit_message_text(text="❌ জিমেইল কাজটি **Reject** করা হয়েছে!", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ ব্যাক (ইনবক্স মেনু)", callback_data="admin_dashboard")]]))
        ...
        await query.edit_message_text(text="✅ উইথড্র রিকুয়েস্ট সফলভাবে **Approve** করা হয়েছে!", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ ব্যাক (ইনবক্স মেনু)", callback_data="admin_dashboard")]]))

    elif data == "wd_reject":
        if pending_withdrawals:
            pending_withdrawals.pop(0)
        await query.edit_message_text(text="❌ উইথড্র রিকুয়েস্ট **Reject** করা হয়েছে!", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ ব্যাক (ইনবক্স মেনু)", callback_data="admin_dashboard")]]))
