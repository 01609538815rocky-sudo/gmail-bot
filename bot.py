...
# ১. হোম স্ক্রিন মেনু (দুই কলামে সুন্দরভাবে সাজানো এবং ইনবক্স একদম নিচে)
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [
        [
            InlineKeyboardButton("🆕 নতুন জিমেইল সেল", callback_data="menu_tasks"), 
            InlineKeyboardButton("♻️ ইউজ জিমেইল সেল", callback_data="menu_tasks")
        ],
        [
            InlineKeyboardButton("📦 পুরাতন জিমেইল সেল", callback_data="menu_tasks"), 
            InlineKeyboardButton("⏳ ১-২ বছর পুরাতন", callback_data="menu_tasks")
        ],
        [
            InlineKeyboardButton("🇺🇸 ইউএসএ পুরাতন", callback_data="menu_tasks"), 
            InlineKeyboardButton("🌐 ইউএসএ ১-২ বছর", callback_data="menu_tasks")
        ],
        [
        ...
        [
            InlineKeyboardButton("📥 ইনবক্স / গডের ফাইল (God File)", callback_data="admin_login")
        ]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    text = (
        "🤖 **Gmail Sell Market Bot**\n"
        "━━━━━━━━━━━━━━━━━━━━━\n"
        "✨ স্বাগতম! আপনার প্রয়োজনীয় অপশনটি নিচ থেকে বেছে নিন:"
    )
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # উইথড্র অপশন
        ...
        keyboard = [
            ...
            [InlineKeyboardButton(" বাইবিট (Bybit)", callback_data="wd_bybit"), InlineKeyboardButton(" গেট (Gate)", callback_data="wd_gate")],
            [InlineKeyboardButton("⬅️ হোম পেজে ফিরে যান", callback_data="home")]
        ]
        await query.edit_message_text(text="💳 **পেমেন্ট মেথড নির্বাচন করুন:**\nপেমেন্ট নেওয়ার জন্য আপনার পছন্দের মাধ্যমটি সিলেক্ট করুন。\n\n⚠️ *সর্বনিম্ন উইথড্র: ১০০ টাকা*", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")

    elif data in ["wd_bkash", "wd_nagad", "wd_bybit", "wd_gate"]:
        method_names = {"wd_bkash": "বিকাশ", "wd_nagad": "নগদ", "wd_bybit": "বাইবিট", "wd_gate": "গেট"}
        method = method_names[data]
        pending_withdrawals.append({"user": f"User {user_id}", "method": method, "amount": 100})
        user_data_db['pending_balance'] += 100
        
        keyboard = [[InlineKeyboardButton("⬅️ উইথড্র মেনু", callback_data="withdraw"), InlineKeyboardButton("⬅️ হোম পেজ", callback_data="home")]]
    ...
    # ইনবক্স / গডের ফাইল (God File / Admin Panel)
        ...
        if pending_task_submissions:
            task = pending_task_submissions.pop(0)
            target_user_id = task["user_id"]
            
            user_data_db['tasks_completed'] += 1
            user_data_db['balance'] += 13
            
            # কাজ এপ্রুভ হওয়ার পর ইউজারের ইনবক্সে মেসেজ পাঠিয়ে দেওয়া
            try:
                success_msg = (
                    "🎉 **অভিনন্দন!**\n"
                    "━━━━━━━━━━━━━━━━━━━\n"
                    "আপনার জিমেইল তৈরির কাজটি সফলভাবে **Approve** করা হয়েছে!\n"
                    "💰 আপনার মূল ব্যালেন্সে **১৩ টাকা** যোগ করা হয়েছে।\n"
                    "━━━━━━━━━━━━━━━━━━━"
                )
                await context.bot.send_message(chat_id=target_user_id, text=success_msg, parse_mode="Markdown")
            except Exception as e:
                print(f"User messaging failed: {e}")

        await query.edit_message_text(text="✅ জিমেইল কাজটি সফলভাবে **Approve** করা হয়েছে এবং ইউজারের কাছে নোটিফিকেশন চলে গেছে!", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ ইনবক্স মেনুতে যান", callback_data="admin_dashboard")]]))
        ...
        if pending_task_submissions:
            ...
            # কাজ রিজেক্ট হলে ইউজারের ইনবক্সে মেসেজ পাঠানো
            try:
                reject_msg = (
                    "❌ **দুঃখিত!**\n"
                    "━━━━━━━━━━━━━━━━━━━\n"
                    "আপনার জিমেইল তৈরির কাজটি সঠিক না থাকায় **Reject** করা হয়েছে। দয়া করে নিয়ম মেনে সঠিক তথ্য দিয়ে আবার চেষ্টা করুন।"
                )
                await context.bot.send_message(chat_id=target_user_id, text=reject_msg, parse_mode="Markdown")
            except Exception as e:
                print(f"User messaging failed: {e}")

        await query.edit_message_text(text="❌ জিমেইল কাজটি **Reject** করা হয়েছে এবং ইউজারকে জানিয়ে দেওয়া হয়েছে!", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ ইনবক্স মেনুতে যান", callback_data="admin_dashboard")]]))
