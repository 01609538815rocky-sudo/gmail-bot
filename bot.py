...
# চ্যাট বক্সের নিচে ফিক্সড বাটনগুলোর লেআউট (স্ক্রিনশটের মতো)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "📋 কাজ":
        ...
        if admin_logged_in.get(user_id, False):
            ...
            await update.message.reply_text("🔐 এডমিন পাসওয়ার্ড (102050) চ্যাটে লিখে পাঠান:")

async def show_admin_panel_msg(update: Update):
    keyboard = [
        [InlineKeyboardButton(f"📥 জমা দেওয়া কাজ ({len(pending_task_submissions)})", callback_data="adm_tasks")],
        [InlineKeyboardButton(f"💸 উইথড্র রিকুয়েস্ট ({len(pending_withdrawals)})", callback_data="adm_withdrawals")]
    ]
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "task_gmail" or data == "next_gmail":
        ...
        if user_id in user_current_gmail:
            ...
            await query.edit_message_text(text="✅ আপনার জিমেইল কাজের রিকুয়েস্ট সফলভাবে এডমিনের কাছে পাঠানো হয়েছে!")
        else:
        ...
        pending_withdrawals.append({"user_id": user_id, "method": method, "amount": withdraw_amount})
        await query.edit_message_text(text=f"✅ সফল! আপনার {method}-এ {withdraw_amount} টাকার উইথড্র রিকুয়েস্ট জমা হয়েছে।")
        ...
        if pending_task_submissions:
            ...
            try:
                ...
                pass
        await query.edit_message_text(text="✅ কাজ সফলভাবে এপ্রুভ করা হয়েছে!")
        ...
        if pending_task_submissions:
            ...
            try:
                ...
                pass
        await query.edit_message_text(text="❌ কাজ রিজেক্ট করা হয়েছে!")
        ...
        if admin_logged_in.get(user_id, False):
            if not pending_withdrawals:
                ...
                wd = pending_withdrawals[0]
                keyboard = [
                    [InlineKeyboardButton("✅ Approve Withdraw", callback_data="wd_approve"), InlineKeyboardButton("❌ Reject Withdraw", callback_data="wd_reject")]
                ]
                text = (
                    "👑 **এডমিন কন্ট্রোল প্যানেল**\n\n"
                    "💸 **উইথড্র রিকুয়েস্ট #১**\n"
        ...
        if pending_withdrawals:
            ...
            try:
                ...
                pass
        await query.edit_message_text(text="✅ উইথড্র এপ্রুভ করা হয়েছে!")
        ...
        if pending_withdrawals:
            ...
            target_u_data['pending_balance'] = max(0, target_u_data['pending_balance'] - wd['amount'])
            target_u_data['balance'] += wd['amount']  # টাকা রিফান্ড
            ...
            try:
                ...
                pass
        await query.edit_message_text(text="❌ উইথড্র রিজেক্ট করা হয়েছে!")
...
if __name__ == "__main__":
    ...
    print("🤖 বট সফলভাবে ২৪ ঘণ্টা লাইভ এবং ফিক্সড মেনুসহ রান হচ্ছে...")
