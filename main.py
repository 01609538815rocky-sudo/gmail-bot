...
completed_tasks = {}  
rejected_tasks = {}   
user_languages = {}   # ইউজারের ভাষা সংরক্ষণ করার জন্য

def get_main_menu(lang="bn"):
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    if lang == "en":
        btn_work = types.KeyboardButton("📋 Task")
...
def send_welcome(message):
    ...
    welcome_text = "✨ **WELCOME TO INSTANT TASK PAY** ✨\n\nআপনাদের স্বাগতম! নিচের অপশনগুলো থেকে নির্বাচন করুন:" if lang == "bn" else "✨ **WELCOME TO INSTANT TASK PAY** ✨\n\nWelcome! Please choose from the options below:"
...
def handle_message(message):
    ...
    if text in ["🔙 ব্যাক", "🔙 Back", "মূল মেনু", "Main Menu"]:
        if chat_id in user_states:
            del user_states[chat_id]
        bot.send_message(chat_id, "মূল মেনুতে ফিরে এসেছেন:" if lang == "bn" else "Returned to main menu:", reply_markup=get_main_menu(lang))
        return
    ...
    if text in ["📋 কাজ", "📋 Task"]:
        ...
        markup.add(types.KeyboardButton("📤 সাবমিট" if lang=="bn" else "📤 Submit"), types.KeyboardButton("🔙 ব্যাক" if lang=="bn" else "🔙 Back"))
        ...
        task_text = (
            ...
            "📋 **Available Gmail Task (Rate: 13 BDT):**\n\n"
            "Please create a Gmail account using the details below and click **'📤 Submit'**:\n\n"
            f"🔹 **First Name:** `{task_data['firstname']}`\n"
            f"🔹 **Last Name:** `{task_data['lastname']}`\n"
            f"🔹 **Gmail:** `{task_data['gmail']}`\n"
            f"🔹 **Password:** `{task_data['pass']}`"
        ...
        if chat_id in user_states and isinstance(user_states[chat_id], dict) and user_states[chat_id].get("state") == "submitting_task":
            ...
            bot.send_message(
                chat_id, 
                "✅ **আপনার জিমেইল সফলভাবে সাবমিট হয়েছে!** 🎉\n\nটাস্কটি পেন্ডিং লিস্টে জমা হয়েছে। এডমিন চেক করার পর ব্যালেন্স যুক্ত হবে।" if lang=="bn" else "✅ **Gmail submitted successfully!** 🎉\n\nTask added to pending list.", 
            ...
            bot.reply_to(message, "আগে '📋 কাজ' থেকে কাজ শুরু করুন।" if lang=="bn" else "Please start a task first.", reply_markup=get_main_menu(lang))

    elif text in ["💰 ব্যালেন্স", "💰 Balance"]:
        ...
        withdraw_intro = (
            ...
            "📌 **উইথড্র নিয়মাবলী:**\n"
            "• বিকাশে মিনিমাম ৳৮০ টাকা\n"
            "• নগদে মিনিমাম ৳৮০ টাকা\n\n"
            "দয়া করে আপনার পেমেন্ট মাধ্যমটি সিলেক্ট করুন:"
        ) if lang == "bn" else (
            f"💳 Your Income Balance: ৳{income:.2f}\n\n"
            "📌 **Withdraw Rules:**\n"
            "• Bkash Minimum ৳80\n"
            "• Nagad Minimum ৳80\n\n"
    ...
    elif text in ["🌐 ভাষা", "🌐 Language"]:
        markup = types.InlineKeyboardMarkup()
        btn_bn = types.InlineKeyboardButton("🇧🇩 বাংলা", callback_data="lang_bn")
        btn_en = types.InlineKeyboardButton("🇺🇸 English", callback_data="lang_en")
        markup.add(btn_bn, btn_en)
        bot.send_message(chat_id, "🌐 দয়া করে আপনার ভাষা নির্বাচন করুন / Please select your language:", reply_markup=markup)
        
    elif text in ["⚙️ এডমিন প্যানেল", "⚙️ Admin Panel"]:
        admin_text = f"⚙️ **এডমিন প্যানেল ও যোগাযোগ**\n\nযে কোনো প্রয়োজনে সরাসরি এডমিনের সাথে যোগাযোগ করুন:\n👉 Telegram: {ADMIN_USERNAME}" if lang=="bn" else f"⚙️ **Admin Panel & Support**\n\nContact Admin:\n👉 Telegram: {ADMIN_USERNAME}"
        ...
        if chat_id in user_states:
            ...
            if isinstance(user_data, str) and user_data.startswith("withdraw_"):
                ...
                if income < 80.0:
                    bot.reply_to(message, "❌ আপনার ব্যালেন্স ৮টি টাকার কম আছে, উইথড্র করতে পারবেন না।" if lang=="bn" else "❌ Insufficient balance for withdrawal.")
                    return
            ...
            bot.reply_to(message, "দয়া করে নিচের মেনু বাটনগুলো ব্যবহার করুন।" if lang=="bn" else "Please use the menu buttons below.", reply_markup=get_main_menu(lang))
...
def callback_query(call):
    ...
    if data == "lang_bn":
        ...
        income = income_balances.get(chat_id, 0.0)
        if income < 80.0:
            bot.answer_callback_query(call.id, "আপনার পর্যাপ্ত ব্যালেন্স নেই (মিনিমাম ৮০ টাকা লাগবে)!", show_alert=True)
            return
        ...
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
        markup.add(types.KeyboardButton("🔙 ব্যাক" if lang=="bn" else "🔙 Back"))
        
        bot.answer_callback_query(call.id)
        bot.send_message(
            chat_id,
            f"📥 **{m_name} উইথড্র ফর্ম**\n\nদয়া করে আপনার একাউন্ট নম্বরটি লিখে পাঠান:" if lang=="bn" else f"📥 **{m_name} Withdraw Form**\n\nPlease send your account number:",
            reply_markup=markup,
            parse_mode="Markdown"
        )
