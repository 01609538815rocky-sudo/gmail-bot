...
def get_main_reply_keyboard():
    keyboard = [
        [KeyboardButton("💳 ডিপোজিট"), KeyboardButton("💸 উইথড্র")],
        [KeyboardButton("💼 কাজসমূহ (Jobs)"), KeyboardButton("👤 প্রোফাইল ও ওয়ালেট")],
        [KeyboardButton("🤝 নেটওয়ার্ক (রেফার)"), KeyboardButton("🛡️ এডমিন প্যানেল")],
        [KeyboardButton("❌ ক্যানসেল")]
    ]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    permanent_markup = get_main_reply_keyboard()
    ...
    if update.message:
        await update.message.reply_text(welcome_text, reply_markup=permanent_markup, parse_mode="Markdown")
        await update.message.reply_text("প্রধান মেন্যু অপশন:", reply_markup=reply_markup)
...
async def job_submit_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✅ আপনার কাজের রিকুয়েস্ট সফলভাবে এডমিনের কাছে পাঠানো হয়েছে!", reply_markup=get_main_reply_keyboard())
...
async def deposit_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    text = "💳 ডিপোজিট প্রক্রিয়া শুরু হয়েছে।\n\nআপনার ফোন নাম্বারটি লিখুন:"
...
async def deposit_screenshot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_photo(chat_id=ADMIN_CHAT_ID, photo=photo_file.file_id, caption=caption, reply_markup=admin_keyboard, parse_mode="Markdown")
    await update.message.reply_text("✅ আপনার ডিপোজিট রিকুয়েস্ট সফলভাবে সাবমিট হয়েছে!", reply_markup=get_main_reply_keyboard())
    return await return_to_home_msg(update, context)
...
async def withdraw_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [[InlineKeyboardButton("❌ ক্যানসেল", callback_data="cancel")]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    text = "💸 উইথড্র প্রক্রিয়া শুরু হয়েছে।\n\nউত্তোলনের পরিমাণ (টাকা) কত তা লিখুন:"
...
async def withdraw_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✅ আপনার উইথড্র রিকুয়েস্ট সফলভাবে সাবমিট হয়েছে!", reply_markup=get_main_reply_keyboard())
    return await return_to_home_msg(update, context)

# এডমিন বাটন হ্যান্ডলার
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = "❌ অপারেশন বাতিল করা হয়েছে। আবার শুরু করতে /start লিখুন বা নিচের মেন্যু ব্যবহার করুন।"
    ...
    if update.callback_query:
        ...
        await query.message.reply_text("প্রধান মেন্যু:", reply_markup=reply_markup)
    else:
        await update.message.reply_text(text, reply_markup=reply_markup)
...
def main():
    ...
    conv_handler = ConversationHandler(
        entry_points=[
            ...
            MessageHandler(filters.Regex("^(💳 ডিপোজিট)$"), deposit_start),
            MessageHandler(filters.Regex("^(💸 উইথড্র)$"), withdraw_start),
            MessageHandler(filters.Regex("^(💼 কাজসমূহ \\(Jobs\\))$"), jobs_menu_view),
            MessageHandler(filters.Regex("^(👤 প্রোফাইল ও ওয়ালেট)$"), profile_view),
            MessageHandler(filters.Regex("^(🤝 নেটওয়ার্ক \\(রেফার\\))$"), network_view),
            MessageHandler(filters.Regex("^(🛡️ এডমিন প্যানেল)$"), admin_panel),
            MessageHandler(filters.Regex("^(❌ ক্যানসেল)$"), cancel),
        ],
        states={
            CHOOSING: [
            ...
            DEPOSIT_PHONE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND & ~filters.Regex("^(💳 ডিপোজিট|💸 উইথড্র|💼 কাজসমূহ \\(Jobs\\)|👤 প্রোফাইল ও ওয়ালেট|🤝 নেটওয়ার্ক \\(রেফার\\)|🛡️ এডমিন প্যানেল|❌ ক্যানসেল)$"), deposit_phone),
                MessageHandler(filters.Regex("^(❌ ক্যানসেল)$"), cancel),
            ],
            DEPOSIT_AMOUNT: [
                MessageHandler(filters.TEXT & ~filters.COMMAND & ~filters.Regex("^(💳 ডিপোজিট|💸 উইথড্র|💼 কাজসমূহ \\(Jobs\\)|👤 প্রোফাইল ও ওয়ালেট|🤝 নেটওয়ার্ক \\(রেফার\\)|🛡️ এডমিন প্যানেল|❌ ক্যানসেল)$"), deposit_amount),
                MessageHandler(filters.Regex("^(❌ ক্যানসেল)$"), cancel),
            ],
            DEPOSIT_SCREENSHOT: [
            ...
            WITHDRAW_AMOUNT: [
                MessageHandler(filters.TEXT & ~filters.COMMAND & ~filters.Regex("^(💳 ডিপোজিট|💸 উইথড্র|💼 কাজসমূহ \\(Jobs\\)|👤 প্রোফাইল ও ওয়ালেট|🤝 নেটওয়ার্ক \\(রেফার\\)|🛡️ এডমিন প্যানেল|❌ ক্যানসেল)$"), withdraw_amount),
                MessageHandler(filters.Regex("^(❌ ক্যানসেল)$"), cancel),
            ],
            WITHDRAW_SCREENSHOT: [
                MessageHandler(filters.PHOTO, withdraw_screenshot),
                MessageHandler(filters.Regex("^(❌ ক্যানসেল)$"), cancel),
            ],
            WITHDRAW_PHONE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND & ~filters.Regex("^(💳 ডিপোজিট|💸 উইথড্র|💼 কাজসমূহ \\(Jobs\\)|👤 প্রোফাইল ও ওয়ালেট|🤝 নেটওয়ার্ক \\(রেফার\\)|🛡️ এডমিন প্যানেল|❌ ক্যানসেল)$"), withdraw_phone),
                MessageHandler(filters.Regex("^(❌ ক্যানসেল)$"), cancel),
            ],
