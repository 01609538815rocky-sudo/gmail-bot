...
<div id="noticeModal" class="modal-overlay">
    <div class="modal-box">
        ...
        <button type="button" id="noticeTgBtn" style="background: linear-gradient(135deg, #0088cc, #00aced); margin-top:10px;">📲 জয়েন করুন 📱</button>
        <button type="button" id="noticeOkBtn" style="background: #f1f2f6; color: #333; margin-top:8px; border:1px solid #ddd;">Ok SIR 🥰</button>
    </div>
</div>
...
<div id="receiptModal" class="modal-overlay" style="display:none;">
    <div class="modal-box" style="text-align: left; padding: 15px;">
        ...
        <button type="button" id="receiptOkBtn" style="margin-top: 0; background: #2196f3;">✔ ঠিক আছে</button>
    </div>
</div>
...
<div class="container">
    <div class="header-top" id="topHeader" style="display:none;">
        ...
        <span id="adminTriggerBtn" style="cursor:pointer; font-size:13px; background:rgba(0,0,0,0.2); padding:4px 8px; border-radius:4px;">⋮ এডমিন</span>
    </div>
    ...
    <div id="splashScreen" class="screen active">
        <div style="text-align:center; margin-top:60px;">
            ...
            <button type="button" id="startBtn" class="action-btn">শুরু করুন</button>
        </div>
    </div>
    ...
    <div id="registerScreen" class="screen">
        ...
        <button type="button" id="registerSubmitBtn">রেজিস্ট্রার করুন</button>
        <div class="switch-text" id="goToLoginText">আগে থেকেই অ্যাকাউন্ট আছে? <span>লগইন করুন</span></div>
    </div>
    ...
    <div id="loginScreen" class="screen">
        ...
        <button type="button" id="loginSubmitBtn">লগইন করুন</button>
        <div class="switch-text" id="goToRegisterText">নতুন অ্যাকাউন্ট খুলতে চান? <span>রেজিস্ট্রেশন করুন</span></div>
    </div>
    ...
    <div id="homeScreen" class="screen">
        ...
        <button type="button" id="goToWalletBtn" style="background: linear-gradient(135deg, #0088cc, #00aced); margin-bottom: 12px; font-size: 14px;">📊 কাজের হিস্টরি ও ওয়ালেট দেখুন</button>

        <div class="task-card" data-task="Gmail" data-reward="13">
        ...
        <div class="task-card" data-task="Facebook" data-reward="10">
        ...
        <div class="task-card" data-task="Instagram" data-reward="10">
    ...
    <div id="taskScreen" class="screen">
        ...
        <button type="button" id="taskSubmitBtn">জমা দিন (পেন্ডিং এ পাঠান)</button>
        <button type="button" id="taskBackBtn" style="background: #7f8c8d;">ফিরে যান</button>
    </div>
    ...
    <div id="balanceScreen" class="screen">
        ...
        <button type="button" id="goToWithdrawBtn">উইথড্র পেজে যান</button>
        <button type="button" id="balanceHomeBackBtn" style="background: #7f8c8d; margin-top:8px;">হোমে ফিরে যান</button>
    </div>
    ...
    <div id="withdrawScreen" class="screen">
        ...
        <button type="button" id="withdrawSubmitBtn">উইথড্র রিকোয়েস্ট পাঠান</button>
        <button type="button" id="withdrawBackBtn" style="background: #7f8c8d; margin-top:8px;">ফিরে যান</button>
    </div>
    ...
    <div id="profileScreen" class="screen">
        <div class="profile-header-card">
            <div class="profile-img-container">
                ...
                <div class="camera-badge" id="cameraBadgeBtn">📷</div>
            </div>
            <input type="file" id="picUploadInput" style="display:none;" accept="image/*">
        ...
        <div class="card" style="text-align: left; padding: 15px;">
            ...
            <button type="button" id="updateProfileBtn">প্রোফাইল আপডেট করুন</button>
        </div>

        <button type="button" id="logoutBtn" style="background: #c0392b; margin-top: 10px;">লগআউট করুন</button>
    </div>
    ...
    <div id="adminScreen" class="screen">
        ...
        <div class="card" style="text-align:left; background:#fff3e0;">
            ...
            <button type="button" id="adminUpdateRatesBtn">রেট আপডেট করুন</button>
        </div>
        ...
        <button type="button" id="adminBackBtn" style="background: #7f8c8d; margin-top:15px;">হোমে ফিরে যান</button>
    </div>
</div>

<div class="nav-bar" id="navBar" style="display: none;">
    <div class="nav-item active-nav" data-nav="homeScreen">🏠<br>হোম</div>
    <div class="nav-item" data-nav="balanceScreen">💳<br>ওয়ালেট</div>
    <div class="nav-item" data-nav="withdrawScreen">💸<br>উইথড্র</div>
    <div class="nav-item" data-nav="profileScreen">👤<br>প্রোফাইল</div>
</div>

<script>
    let currentUser = null;
    let taskRates = { gmail: 13, facebook: 10, instagram: 10 };
    let currentTaskType = 'Gmail';
    let currentReward = 13;
    ...
    function bindClick(elId, callback) {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        // Modal Actions
        bindClick('noticeTgBtn', () => { window.location.href='https://t.me/'; });
        bindClick('noticeOkBtn', () => { document.getElementById('noticeModal').style.display = 'none'; });
        bindClick('receiptOkBtn', () => { 
        ...
        bindClick('goToLoginText', () => showScreen('loginScreen'));
        bindClick('goToRegisterText', () => showScreen('registerScreen'));
        bindClick('adminTriggerBtn', checkAdminPassword);

        // Forms & Submissions
        bindClick('registerSubmitBtn', handleRegister);
        bindClick('loginSubmitBtn', handleLogin);
        bindClick('goToWalletBtn', () => { showScreen('balanceScreen'); updateBalanceUI(); });
        bindClick('taskSubmitBtn', submitTask);
        bindClick('taskBackBtn', () => showScreen('homeScreen'));
        bindClick('goToWithdrawBtn', () => showScreen('withdrawScreen'));
        bindClick('balanceHomeBackBtn', () => showScreen('homeScreen'));
        bindClick('withdrawSubmitBtn', handleWithdraw);
        bindClick('withdrawBackBtn', () => showScreen('homeScreen'));
        
        // Profile
        bindClick('cameraBadgeBtn', () => document.getElementById('picUploadInput').click());
        document.getElementById('picUploadInput').addEventListener('change', handleImageUpload);
        bindClick('updateProfileBtn', updateUserProfile);
        bindClick('logoutBtn', logout);

        // Admin
        bindClick('adminUpdateRatesBtn', updateTaskRates);
        bindClick('adminBackBtn', () => showScreen('homeScreen'));

        // Task Cards Dynamic Binding
        document.querySelectorAll('.task-card').forEach(card => {
            const openTaskAction = () => {
                currentTaskType = card.getAttribute('data-task');
                currentReward = Number(card.getAttribute('data-reward'));
                document.getElementById('taskTitle').innerText = currentTaskType + ' Task (রেট: ৳' + currentReward + ')';
                document.getElementById('taskInputName').value = currentUser ? currentUser.name : '';
                showScreen('taskScreen');
            };
            card.addEventListener('click', openTaskAction);
        ...
        document.querySelectorAll('.nav-item').forEach(nav => {
            const navAction = () => {
                ...
                showScreen(target);
                if(target === 'balanceScreen') updateBalanceUI();
                if(target === 'profileScreen') loadProfileData();
            };
    ...
    async function submitTask() {
        ...
        try {
            const res = await fetch('/api/submit-task', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ phone: currentUser.phone, name: currentUser.name, taskType: currentTaskType, submitterName, gmailAddress, gmailPass, reward: currentReward })
            });
            const data = await res.json();
            alert(data.message);
            if(data.success) { 
        ...
        try {
            ...
            data.tasks.filter(t => t.status === 'pending').forEach(t => {
                listHTML += '<div class="card" style="text-align:left; font-size:12px; background:#f9f9f9;">' +
                    ...
                    '<div style="margin-top:8px; display:flex; gap:5px;">' +
                        '<button type="button" class="action-btn" onclick="reviewTask(\'' + t.id + '\', \'approve\')" style="padding:6px; margin:0;">Approve</button>' +
                        '<button type="button" class="action-btn" onclick="reviewTask(\'' + t.id + '\', \'reject\')" style="padding:6px; margin:0; background:#c0392b;">Reject</button>' +
                    '</div>' +
                '</div>';
            });
            ...
            data.withdraws.filter(w => w.status === 'pending').forEach(w => {
                wdHTML += '<div class="card" style="text-align:left; font-size:12px; background:#f1f8e9;">' +
                    ...
                    '<div style="margin-top:8px; display:flex; gap:5px;">' +
                        '<button type="button" class="action-btn" onclick="reviewWithdraw(\'' + w.id + '\', \'approve\')" style="padding:6px; margin:0;">Approve</button>' +
                        '<button type="button" class="action-btn" onclick="reviewWithdraw(\'' + w.id + '\', \'reject\')" style="padding:6px; margin:0; background:#c0392b;">Reject</button>' +
                    '</div>' +
                '</div>';
            });
