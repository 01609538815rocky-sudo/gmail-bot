...
<div id="noticeModal" class="modal-overlay">
    <div class="modal-box">
        ...
        <button onclick="window.location.href='https://t.me/';" style="background: linear-gradient(135deg, #0088cc, #00aced); margin-top:10px;">📲 জয়েন করুন 📱</button>
        <button onclick="document.getElementById('noticeModal').style.display='none';" style="background: #f1f2f6; color: #333; margin-top:8px; border:1px solid #ddd;">Ok SIR 🥰</button>
    </div>
</div>
...
<div id="receiptModal" class="modal-overlay" style="display:none;">
    <div class="modal-box" style="text-align: left; padding: 15px;">
        ...
        <button onclick="document.getElementById('receiptModal').style.display='none'; showScreen('homeScreen');" style="margin-top: 0; background: #2196f3;">✔ ঠিক আছে</button>
    </div>
</div>
...
<script>
    ...
    function initApp() {
        ...
        if(startBtn) startBtn.onclick = checkFirstTimeUser;
        ...
        if(toLoginText) toLoginText.onclick = showLoginScreen;
        
        const toRegText = document.getElementById('toRegText');
        if(toRegText) toRegText.onclick = showRegisterScreen;
        
        const goToBalanceBtn = document.getElementById('goToBalanceBtn');
        if(goToBalanceBtn) goToBalanceBtn.onclick = () => showScreen('balanceScreen');
        
        const taskSubmitBtn = document.getElementById('taskSubmitBtn');
        if(taskSubmitBtn) taskSubmitBtn.onclick = submitTask;
        
        const taskBackBtn = document.getElementById('taskBackBtn');
        if(taskBackBtn) taskBackBtn.onclick = () => showScreen('homeScreen');
        
        const goToWithdrawBtn = document.getElementById('goToWithdrawBtn');
        if(goToWithdrawBtn) goToWithdrawBtn.onclick = () => showScreen('withdrawScreen');
        
        const balHomeBtn = document.getElementById('balHomeBtn');
        if(balHomeBtn) balHomeBtn.onclick = () => showScreen('homeScreen');
        
        const withdrawSubmitBtn = document.getElementById('withdrawSubmitBtn');
        if(withdrawSubmitBtn) withdrawSubmitBtn.onclick = handleWithdraw;
        
        const wdHomeBtn = document.getElementById('wdHomeBtn');
        if(wdHomeBtn) wdHomeBtn.onclick = () => showScreen('homeScreen');
        
        const cameraBadgeBtn = document.getElementById('cameraBadgeBtn');
        if(cameraBadgeBtn) cameraBadgeBtn.onclick = () => document.getElementById('picUploadInput').click();
        
        const picUploadInput = document.getElementById('picUploadInput');
        if(picUploadInput) picUploadInput.onchange = handleImageUpload;
        
        const profileUpdateBtn = document.getElementById('profileUpdateBtn');
        if(profileUpdateBtn) profileUpdateBtn.onclick = updateUserProfile;
        
        const logoutBtn = document.getElementById('logoutBtn');
        if(logoutBtn) logoutBtn.onclick = logout;
        
        const adminBtn = document.getElementById('adminBtn');
        if(adminBtn) adminBtn.onclick = checkAdminPassword;
        
        const adminUpdateRatesBtn = document.getElementById('adminUpdateRatesBtn');
        if(adminUpdateRatesBtn) adminUpdateRatesBtn.onclick = updateTaskRates;
        
        const adminHomeBtn = document.getElementById('adminHomeBtn');
        if(adminHomeBtn) adminHomeBtn.onclick = () => showScreen('homeScreen');
